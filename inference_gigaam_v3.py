...
"""VAD-only chunked transcription for Salute GigaAM v3 (e2e RNNT, built-in punctuation).

- Silero VAD only (no Whisper VAD).
- Uses raw VAD spans (~TARGET_SPEECH_SEC).
- Writes segments.jsonl for per-chunk output and dialog.jsonl.
- Keeps the same VAD packing/merge logic as v2.
- Punctuation comes from the model itself (no external punctuator)."""
...
# МОДЕЛЬ/ЯЗЫК
HF_MODEL_ID: str = "ai-sage/GigaAM-v3"           # Hugging Face repo
MODEL_NAME: str = "e2e_rnnt"                     # revision ?? HF (e2e ????? ? ???????????)
LANG: str = "ru"                             # ????? "ru" ??? ???????
# VAD (Silero)
...
import json, os, sys, gc, io, tempfile, subprocess, shutil
...
def ensure_wav_cache(src: Path, target_sr: int = 16000) -> Path:
    """Ensure there is a cached wav16k mono next to the source (stem_16k.wav)."""
    if src.suffix.lower() == ".wav":
        return src

    target = src.with_name(f"{src.stem}_16k.wav")
    if target.exists():
        return target

    if not shutil.which("ffmpeg"):
        if DEBUG:
            print(f"[WARN] ffmpeg not found, using original file: {src}")
        return src
    ...
    try:
        ...
        if DEBUG:
            print(f"[WARN] ffmpeg cache convert failed for {src}: {e}")
        try:
...
def _read_audio_16k_mono(path: Path, target_sr: int = 16000) -> tuple[np.ndarray, int]:
    ...
    # 2) Fallback: torchaudio (sox / ffmpeg backends)
    if data is None and ta is not None:
        try:
            ...
            data = None
            last_err = e
            print(f"[WARN] torchaudio read failed for {path}: {last_err}")

    # 3) Fallback: ffmpeg CLI to wav (only if still unresolved)
    if data is None and shutil.which("ffmpeg"):
        tmp_wav = Path(tempfile.gettempdir()) / f"tmp_ffmpeg_{path.stem}_{os.getpid()}.wav"
        cmd = [
            "ffmpeg",
            "-y",
            "-i",
        ...
        try:
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            data, sr = sf.read(str(tmp_wav), always_2d=False)
        except Exception as e:
            data = None
            print(f"[ERROR] ffmpeg fallback failed for {path}: {e}")
        finally:
            try:
    ...
    if sr != target_sr:
        ...
        # Prefer torchaudio resample if available (streaming-friendly), else SciPy for shorter clips.
        if ta is not None:
            try:
                tensor = torch.from_numpy(data)
                tensor = ta.functional.resample(tensor, sr, target_sr)
                data = tensor.cpu().numpy().astype(np.float32, copy=False)
                sr = target_sr
                return data, sr
            except Exception:
                pass

        # Avoid FFT-based resample for long clips to reduce peak memory.
        if resample is not None and n_in < 2_000_000:  # ~125s at 16 kHz
...
def load_hf_gigaam_model(model_id: str, revision: str):
    """Load v3 model from Hugging Face (trust_remote_code) and move to CUDA if available."""
    if AutoModel is None:
        raise SystemExit(
            "Transformers is required for GigaAM v3.\n"
            "pip install transformers>=4.57.1"
        ) from _IMPORT_ERR  # type: ignore[arg-type]
...
def transcribe_file_sequential(
    ...
    *,
    ...
    for i, ((s0, s1), seg) in enumerate(zip(chunks, segments)):
        ...
        # Подготовка источника: всегда используем временный файл для совместимости с моделью
...
def process_single_file(
    ...
    if local_model is None:
        local_model = load_hf_gigaam_model(HF_MODEL_ID, MODEL_NAME)
...
def main():
    ...
    model = load_hf_gigaam_model(HF_MODEL_ID, MODEL_NAME)
    ...
    for root, dirs, files in os.walk(input_path):
        ...
        for src in audio_files:
            conv = ensure_wav_cache(src, target_sr=16000)
            converted_files.append((src, conv))
        ...
        # ???????????? ?????? ????? (?????????? ????????????????? ????, ?? ????????? ??? ?????????)
        for src_path, proc_path in converted_files:
            ...
            results[orig_name] = full_text

            if local_segments_dir:
                seg_file = local_segments_dir / f"{src_path.stem}_segments.jsonl"
        ...
        # External punctuation step skipped: v3 e2e model already returns punctuated text.
...
def _convert_to_wav_16k_mono(src: Path, repo_root: Path) -> Path:
    """
    Pre-decode original audio to WAV 16k mono via ffmpeg.
    Returns destination path (may be original if conversion fails).
    """
    if not shutil.which("ffmpeg"):
        return src

    tmp_dir = repo_root / ".tmp" / "predecode_wav"
    tmp_dir.mkdir(parents=True, exist_ok=True)
    dst = tmp_dir / f"{src.stem}_16k.wav"
    ...
    try:
        ...
        if DEBUG:
            print(f"[WARN] ffmpeg predecode failed for {src}: {e}")
        try:
